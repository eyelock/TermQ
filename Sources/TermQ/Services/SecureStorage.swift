...
public actor SecureStorage {
    ...
    /// Designated init. Tests inject `InMemoryEncryptionKeyStore` and a temp
    /// directory; production callers use the default singleton (`.shared`).
    init(
        keyStore: any EncryptionKeyStore = LiveEncryptionKeyStore(
            service: AppProfile.Services.keychainService,
            account: "encryption-key",
            debugKeyDirectory: DataDirectoryManager.url
        ),
        configDirectory: URL = DataDirectoryManager.url
    ) {
    ...
    /// Gets the current config directory URL.
    ...
    private func getOrCreateEncryptionKey() throws -> SymmetricKey {
        if let existingKey = try? keyStore.load() { return existingKey }
        let newKey = SymmetricKey(size: .bits256)
        try keyStore.store(newKey)
        return newKey
    }
