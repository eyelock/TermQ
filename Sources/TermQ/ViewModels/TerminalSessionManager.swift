...
class TerminalSessionManager: ObservableObject {
    ...
    func selectPane(direction: PaneDirection, cardId: UUID) {
        ...
        // select-pane doesn't trigger %layout-change, so the new active pane won't
        // arrive via onLayoutChange. Query list-panes immediately so the parser
        // picks up the updated #{pane_active} flag, then promote it to activePaneId.
        controlSession.sendCommand(
            "list-panes -F '#{pane_id} #{pane_width} #{pane_height} #{pane_left} #{pane_top} #{pane_active}'"
        )
        Task { @MainActor [weak self] in
            try? await Task.sleep(nanoseconds: 50_000_000)  // 50ms for list-panes response
            guard let self else { return }
            if let activePane = controlSession.parser.panes.first(where: { $0.isActive }) {
                self.sessions[cardId]?.activePaneId = activePane.id
            }
