...
- **Link PRs into a Stack** *(GitHub backend)* — adopts existing branches into a stack on GitHub. Given its own menu item rather than folded into Track Branch, because unlike that local-only operation it **creates a pull request for any branch that lacks one**; the sheet marks each branch and counts the PRs it will open.
- **Check Out Whole Stack** *(GitHub backend)* — in the Remote PRs feed, a PR belonging to a GitHub stack can bring down the entire stack rather than one branch. Checking out a single PR of a stack leaves you with a branch whose parent isn't present, so the diff reads against the wrong base and the stack UI stays inert.
...
- **A branch shows its pull request straight after Link, without a manual refresh.** `gh stack link` states outright that it "does not rely on gh-stack local tracking state" — it registers the stack on GitHub and writes nothing locally, so the tracking file still said "no PR" for a branch that had just had one opened for it. Every gh-stack command that would reconcile this carries a side effect TermQ must not apply unasked: `sync` force-pushes every branch, `checkout` moves HEAD, and `init` refuses outright once a branch is tracked. TermQ already holds the repository's open pull requests, keyed by head branch, so a branch the provider reports no change request for now shows the one we already know about. It only ever fills in a missing value — a change request the provider reported always wins, since it carries the URL and the real status (merged, closed) that a list of open pull requests cannot.

- **The repo refresh button no longer force-pushes.** Refreshing a stacked repo runs the provider's sync rather than a plain fetch, because a fetch alone leaves the stack stale after a downstack merge. That is free for git-spice, whose `repo sync` is local-only — but `gh stack sync` force-pushes every branch and rewrites the stack on GitHub, which is precisely why Sync is a confirmed action. Reaching it through ↻ performed that operation unasked, and it was not theoretical: refreshing after a merge silently force-pushed and deleted the merged branches, which is how it was found. Repos whose sync reaches the remote now take the plain fetch on refresh; the stale-after-merge case is reached through Sync, where the branches that will be pushed are named first.

- **A stack whose branches have all been deleted no longer sits in the sidebar.** `gh stack merge` deletes every branch in the stack, locally and on the remote, but leaves the stack in its tracking file. Read back literally that file describes a live stack, so after a successful merge the sidebar kept showing one — offering actions against branches git no longer had. Nothing needed syncing; the cleanup had already happened. Tracked branches that do not exist in the repository are now dropped, so a merged stack disappears with its branches. If the branch list can't be read at all, the tracking file is trusted as-is rather than blanking every stack in the sidebar on a failed subprocess.

- **A merged stack no longer keeps rendering as live.** gh-stack's tracking file records a branch's pull request number and nothing about its state, and the stack graph is built from that file, so merging left every pull request in the stack showing as open — the merge confirmation would say "Already merged" against all of them while the sidebar behind it still showed them live. TermQ now reads which pull requests are merged and reports them as merged. Fetched rather than remembered: noting only what this session merged is wrong after a restart, and wrong for a stack merged on github.com or by a teammate. It is one bounded `gh pr list --state merged` per repository, TTL'd and coalesced, because it is paid on every stack refresh — unlike the per-pull-request readiness fetch, which is paid once when a merge sheet opens. A failed fetch keeps what is already known rather than flipping merged pull requests back to open.

- **The merge confirmation no longer shows a green tick against a pull request that cannot merge.** A PR sitting above the blocker is ready in itself, so it was ticked like any other, while the summary underneath explained that nothing above the blocker can merge. It now gets a neutral mark: ready, blocked, and mergeable are three states, not two.

- **Counts in the stack sheets now agree grammatically.** The Link sheet read "Creates 1 new pull requests on GitHub." — and a count of one is the *common* case there, since a stack usually has a single branch without a pull request yet. TermQ had no plural infrastructure at all: every count-bearing string hardcoded the plural form, visibly enough that some had been papered over as "worktree(s)". The six strings the stack sheets format now resolve through `.stringsdict` files carrying a form per plural category, in the 29 languages whose grammar needs one — the other eleven either have a single category (Japanese, Korean, Chinese, Thai, Vietnamese, Indonesian, Malay) or take the singular after any numeral (Hungarian, Turkish), and were already correct. Each form is derived from the existing translation, so the wording is the translator's and only the number agreement changed. Selecting a language explicitly in Settings previously bypassed this entirely: that path hand-parses `Localizable.strings`, which holds one form per key, so plural keys now go through a real bundle and let Foundation apply the language's rules.
