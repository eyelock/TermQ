...
- **A merged stack no longer keeps rendering as live.** `gh stack merge` records nothing locally — the tracking file still describes an open stack afterwards, and the sidebar's graph is built from that file, so every pull request in a stack that had just merged still showed as open until a `gh stack sync` rewrote it. Sync force-pushes, so TermQ asks before running it, which left the terminal action looking unfinished. TermQ performed the merge, so it now records what it merged and reports those change requests as merged. Recorded rather than inferred: `gh stack view --json` does carry `pr.state`, but building the graph is deliberately offline — tracking file plus `git worktree list`, on every refresh — and putting a network round-trip there to learn something TermQ already knows would be the wrong trade.
